...
async def _crawl_and_index(
    ...
    try:
        ...
        if not pages:
            await db.update_crawl_job(job_id, status="failed", error="No pages found")
            return
...
async def start_crawl(body: CrawlRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        # Create job
        job_id = await db.create_crawl_job(body.url)
...
async def get_latest_crawl():
    ...
    try:
        db = await get_database()
        job = await db.get_latest_crawl_job()
...
async def get_pages():
    ...
    try:
        db = await get_database()
        pages = await db.get_all_pages(status="indexed")
