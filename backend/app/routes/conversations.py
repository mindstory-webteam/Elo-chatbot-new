...
async def _resolve_conversation_site_scope(
    ...
    """
    Returns (single_site_id, site_ids) for the database query:
...
async def _ensure_conversation_access(user: dict, db, session_id: str) -> dict:
    conv = await db.get_conversation_full(session_id)
    if not conv:
...
async def _validate_export_access(user: dict, db, request: ExportRequest) -> None:
    ...
    if request.site_id:
        site = await db.get_site(request.site_id)
        if not site or not can_view_site(user, site):
            raise HTTPException(status_code=403, detail="Access denied")
    if request.session_ids:
        for sid in request.session_ids:
            conv = await db.get_conversation_full(sid)
            if not conv:
                raise HTTPException(status_code=404, detail="Conversation not found")
            csid = conv.get("site_id")
            if not csid:
                raise HTTPException(status_code=403, detail="Access denied")
            site = await db.get_site(csid)
            if not site or not can_view_site(user, site):
                raise HTTPException(status_code=403, detail="Access denied")
...
async def list_conversations(
    ...
    sid, sids = await _resolve_conversation_site_scope(user, db, site_id)
    ...
    if sid:
        conversations, total = await db.get_conversations_paginated(site_id=sid, **kw)
    elif sids is not None:
        conversations, total = await db.get_conversations_paginated(site_ids=sids, **kw)
    else:
        conversations, total = await db.get_conversations_paginated(**kw)
...
async def search_conversations(
    ...
    if sid:
        conversations, total = await db.search_conversations(
...
async def auto_close_conversations(
    ...
    db = await get_database()
    closed_count = await db.auto_close_inactive_conversations(body.days_inactive)
...
async def update_conversation_status(
    ...
    db = await get_database()
    await _ensure_conversation_access(user, db, session_id)
    updated = await db.update_conversation_status(session_id, request.status)
    if not updated:
        raise HTTPException(status_code=404, detail="Conversation not found")
...
async def update_conversation_priority(
    ...
    db = await get_database()
    await _ensure_conversation_access(user, db, session_id)
    updated = await db.update_conversation_priority(session_id, request.priority)
    if not updated:
        raise HTTPException(status_code=404, detail="Conversation not found")
...
async def update_conversation_tags(
    ...
    db = await get_database()
    await _ensure_conversation_access(user, db, session_id)
    updated = await db.update_conversation_tags(session_id, request.tags)
    if not updated:
        raise HTTPException(status_code=404, detail="Conversation not found")
...
async def add_conversation_note(
    ...
    db = await get_database()
    await _ensure_conversation_access(user, db, session_id)
    note = await db.add_conversation_note(session_id, request.content)
    return note
...
async def update_conversation_note(
    ...
    db = await get_database()
    await _ensure_conversation_access(user, db, session_id)
    updated = await db.update_conversation_note(session_id, note_id, request.content)
    if not updated:
...
async def delete_conversation_note(
    ...
    db = await get_database()
    await _ensure_conversation_access(user, db, session_id)
    deleted = await db.delete_conversation_note(session_id, note_id)
    if not deleted:
        raise HTTPException(status_code=404, detail="Note not found")
...
async def update_conversation_visitor(
    ...
    db = await get_database()
    await _ensure_conversation_access(user, db, session_id)
    updated = await db.update_conversation_visitor(
        session_id,
...
async def mark_conversation_read(session_id: str, user: dict = Depends(require_auth)):
    ...
    db = await get_database()
    await _ensure_conversation_access(user, db, session_id)
    await db.mark_conversation_read(session_id)
...
async def set_conversation_rating(
    ...
    db = await get_database()
    await _ensure_conversation_access(user, db, session_id)
    updated = await db.set_conversation_rating(session_id, request.rating)
    if not updated:
        raise HTTPException(status_code=404, detail="Conversation not found")
...
async def get_conversation(session_id: str, user: dict = Depends(require_auth)):
    ...
    conv = await _ensure_conversation_access(user, db, session_id)
...
async def delete_conversation(session_id: str, user: dict = Depends(require_auth)):
    ...
    deleted = await db.clear_conversation(session_id)
    if not deleted:
        raise HTTPException(status_code=404, detail="Conversation not found")
...
async def bulk_delete_conversations(
    ...
    for sid in request.session_ids:
        await _ensure_conversation_access(user, db, sid)

    deleted_count = await db.delete_conversations_bulk(request.session_ids)
...
async def export_conversations(
    ...
    """
    db = await get_database()
    await _validate_export_access(user, db, request)

    conversations = await db.get_conversations_for_export(
