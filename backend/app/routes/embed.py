...
async def crawl_and_index_site(url: str, site_id: str, max_pages: int = 50):
    ...
    try:
        ...
        job_id = await db.create_crawl_job(url)
        ...
        if not pages:
            ...
            await db.update_crawl_job(job_id, status="failed", error=error_msg)
        ...
        if job_id:
            await db.update_crawl_job(job_id, status="failed", error=str(e))
...
async def setup_chatbot(
    ...
    existing = await db.get_site(site_id)
    ...
    if sri_hash:
        embed_script = f'''<!-- elo Widget (Secure) -->
<script>
(function() {{
