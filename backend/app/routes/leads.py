...
async def _require_site_leads_access(site_id: str, user: dict, db) -> None:
...
async def capture_lead(request: Request, body: LeadCreate):
    ...
    existing = await db.get_lead_by_session(body.site_id, body.session_id)
    if existing:
        return {
            "success": True,
            "lead_id": existing["id"],
...
async def check_lead_exists(site_id: str, session_id: str):
    ...
    lead = await db.get_lead_by_session(site_id, session_id)
...
async def get_site_leads(
    ...
    db = await get_database()
    await _require_site_leads_access(site_id, user, db)
...
async def export_leads(
    ...
    leads = await db.get_all_leads_for_export(site_id)
...
async def get_leads_count(
    ...
    count = await db.get_leads_count(site_id)
...
async def delete_lead(
    ...
    await _require_site_leads_access(lead["site_id"], user, db)
