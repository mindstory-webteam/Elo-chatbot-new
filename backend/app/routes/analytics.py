...
async def get_analytics_overview(
    ...
    try:
        ...
        conversations = await db.get_all_conversations(site_id=site_id)
        ...
        sites_count = await db.count_sites(status="ready")

        # Get handoff stats
        total_handoffs = await db.count_handoffs(site_id=site_id)
        resolved_handoffs = await db.count_handoffs(
            site_id=site_id, status="resolved"
        )
...
async def get_conversations_by_site():
    ...
    try:
        ...
        rows = await db.get_conversation_counts_by_site(limit=20)

        # Fetch site names in bulk
        site_ids = [r["site_id"] for r in rows if r["site_id"]]
        sites_map: dict = await db.get_site_names(site_ids)
