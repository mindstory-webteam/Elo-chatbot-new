...
async def check_handoff_suggestion(site_id: str, answer: str, confidence: float) -> tuple[bool, str]:
    ...
    config = await db.get_site_handoff_config(site_id) if site_id else None
...
async def get_history(session_id: str):
    ...
    try:
        db = await get_database()
        messages = await db.get_conversation_history(session_id, limit=50)
...
async def clear_history(session_id: str):
    ...
    try:
        db = await get_database()
        result = await db.clear_conversation(session_id)
...
async def submit_feedback(
    ...
    try:
        db = await get_database()
        result = await db.add_feedback_by_index(session_id, message_index, feedback)
