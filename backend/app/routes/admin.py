...
async def health_check():
    ...
    try:
        ...
        overall = "healthy" if all(
            ...
            for s in [database_status, vector_store_status, ollama_status]
        ) else "degraded"
...
async def get_stats():
    ...
    try:
        ...
        # Get counts
        page_count = await db.get_page_count()
        vector_stats = vector_store.get_collection_stats()
        
        # Get conversation stats
        conversations = await db.get_all_sessions(limit=1000)
        total_conversations = len(conversations)
        
        # Get latest crawl
        latest_job = await db.get_latest_crawl_job()
