...
async def create_handoff(request: Request, body: HandoffRequest):
    ...
    existing = await db.get_handoff_by_session(body.session_id, active_only=True)
    if existing:
        # Visitor tapped "connect to agent" again while still pending — bump queue ordering + dashboard "new" signal.
        if existing.get("status") == "pending":
            await db.bump_handoff_visitor_requeue_pending(existing["handoff_id"])
        return {
...
async def get_handoff(handoff_id: str):
    ...
    handoff = await db.get_handoff_session(handoff_id)
    if not handoff:
...
async def get_handoff_messages(
    ...
    result = await db.get_handoff_messages(handoff_id, since)
    if not result:
        raise HTTPException(status_code=404, detail="Handoff not found")
...
async def abandon_handoff_public(
    ...
    updated = await db.update_handoff_status(handoff_id=handoff_id, status="abandoned")
    if not updated:
...
async def check_availability(site_id: str):
    ...
    result = await db.check_business_hours(site_id)
...
async def get_handoff_queue(
    ...
    if site_id == "all":
        if is_admin(user):
            handoffs, total, pending_count, active_count = await db.get_handoff_queue(
                site_id=None,
                site_ids=None,
                status=status_list,
                page=page,
...
async def get_handoff_full(
    ...
    site = await db.get_site(handoff["site_id"])
...
async def update_handoff_status(
    ...
    updated = await db.update_handoff_status(
        handoff_id=handoff_id,
...
async def assign_handoff_to_agent(
    ...
    updated = await db.assign_handoff_agent(
        handoff_id=handoff_id,
...
async def send_agent_message(
    ...
    if handoff["status"] == "pending":
        await db.update_handoff_status(
            handoff_id=handoff_id,
            status="active",
            agent_id=str(user["_id"]),
            agent_name=user.get("name") or user.get("email", "Agent")
    ...
    updated_handoff = await db.get_handoff_session(handoff_id)
    return {
        "success": True,
        "message": message,
...
async def get_handoff_config(
    ...
    config = await db.get_site_handoff_config(site_id)
    return config or {}
...
async def update_handoff_config(
    ...
    success = await db.update_site_handoff_config(site_id, config_dict)
            ...
            while True:
                ...
                try:
                    ...
                    if effective_site_id:
                        site = await db.get_site(effective_site_id)
                        ...
                        sites_list = await db.list_sites(user_id=str(user["_id"]))
    ...
    async def generator():
        ...
        try:
            while True:
                ...
                try:
                    result = await db.get_handoff_messages(handoff_id)
                    if not result:
                        yield f"event: error\ndata: {json.dumps({'error': 'Handoff not found'})}\n\n"
                        break
