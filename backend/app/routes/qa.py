...
async def create_qa_pair(
    ...
    qa_pair = await db.create_qa_pair(qa_data)
...
async def list_qa_pairs(
    ...
    qa_pairs, total = await db.get_qa_pairs(
        site_id=site_id,
        page=page,
        limit=limit,
        search=search,
...
async def get_qa_stats(
    ...
    stats = await db.get_qa_stats(site_id)
...
async def update_qa_pair(
    ...
    updated_qa = await db.update_qa_pair(qa_id, updates)
    if not updated_qa:
...
async def delete_qa_pair(
    ...
    deleted = await db.delete_qa_pair(qa_id)
    if not deleted:
...
async def create_qa_from_conversation(
    ...
    conv = await db.get_conversation_full(request.session_id)
    if not conv:
    ...
    await db.mark_message_has_qa(request.session_id, request.message_index, qa_pair["id"])
...
async def toggle_qa_pair(
    ...
    updated_qa = await db.update_qa_pair(qa_id, {"enabled": new_enabled})
