...
<h1><img src="docs/images/elo-logo.svg" alt="elo" width="280" height="62" /></h1>
...
## 🔄 elo vs Commercial Alternatives

Many businesses rely on cloud-based chatbot services that charge recurring subscription fees and store customer data on third-party servers. elo offers a fundamentally different approach:

| Aspect | Commercial SaaS Platforms | elo |
|--------|---------------------------|----------|
...
elo requires self-hosting infrastructure and technical expertise to deploy and maintain. Commercial platforms offer managed hosting and support, which may be preferable for teams without DevOps resources.
...
| [📸 Screenshots](#-screenshots) | [🔄 Comparison](#-elo-vs-commercial-alternatives) | [🆕 Recent updates](#-recent-updates) |
...
elo uses a **Provider Pattern** for all infrastructure components, making it easy to swap providers without changing business logic.
...
│   │   │   ├── database/          # Database providers (PostgreSQL)
...
# 3. Start PostgreSQL and create the database + role
#    (see "PostgreSQL Setup" below for the full commands)
createdb elo
...
> **Seeing “It works!”?** That is Apache on port **80**, not elo. Use **http://localhost:8000** above, or run `./scripts/run-elo.sh` from the repo root (opens the browser). To serve elo on plain `http://localhost`, see [deploy/APACHE_PROXY.md](deploy/APACHE_PROXY.md).
...
elo stores all application data (conversations, sites, users, leads, Q&A
pairs, crawl jobs) in PostgreSQL 14 or newer. Vector embeddings remain in the
configured vector store (FAISS by default), not in PostgreSQL.
...
# Docker (quickest option for local development)
docker run --name elo-pg -e POSTGRES_PASSWORD=elo \
  -e POSTGRES_USER=elo -e POSTGRES_DB=elo \
  -p 5432:5432 -d postgres:16
```

**2. Create the database and a dedicated role** (skip if you used Docker above)

```bash
sudo -u postgres psql <<'SQL'
CREATE ROLE elo LOGIN PASSWORD 'change-me';
CREATE DATABASE elo OWNER elo;
SQL
```

The `pgcrypto` extension is required for UUID primary keys. The first migration
creates it automatically, which needs a superuser or a role with `CREATE`
privilege on the database. If your role is unprivileged, have an administrator
run `CREATE EXTENSION IF NOT EXISTS pgcrypto;` against the database once,
before the first migration.

**3. Point the application at it**

Set `POSTGRES_URL` in `backend/.env` (or the discrete `POSTGRES_HOST` /
`POSTGRES_PORT` / `POSTGRES_DB` / `POSTGRES_USER` / `POSTGRES_PASSWORD`
variables — `POSTGRES_URL` wins when both are present):

```bash
DATABASE_PROVIDER=postgresql
POSTGRES_URL=postgresql://elo:change-me@localhost:5432/elo
```

### Database Migrations

Migrations are plain `.sql` files in `backend/app/database/migrations/`, applied
in filename order and recorded in a `schema_migrations` table so each runs
exactly once. An advisory lock serialises concurrent runners, so it is safe for
several workers to start at the same time.
...
The runner reads the same `.env` as the application and does not import the
LLM/vector-store stack, so it can run in a slim deploy container.

`POSTGRES_AUTO_MIGRATE=true` (the default) also applies pending migrations when
the app starts, which is convenient locally. **Set it to `false` in production**
and run `python -m app.database.migrate` as an explicit, reviewable deploy step
instead.

To add a schema change, create the next numbered file — for example
`0002_add_widget_theme.sql` — and re-run the command. Never edit a migration
that has already been applied; the runner compares checksums and will warn you
if one changes.
...
python -m app.database.migrate      # ensure the schema is current
uvicorn app.main:app --reload --host 0.0.0.0 --port 8000
```

On first start the app creates the admin account from `ADMIN_EMAIL` /
`ADMIN_PASSWORD` if no admin exists. Confirm the database is healthy at
`GET /api/admin/health`, which reports a `database` field.

> **Note on migrating existing data.** These migrations create a fresh schema;
> they do not copy documents out of an existing MongoDB deployment. If you are
> moving a live installation, export each collection and load it into the
> matching table — the field names are preserved, so the mapping is direct.
...
DATABASE_PROVIDER=postgresql
POSTGRES_URL=postgresql://elo:change-me@localhost:5432/elo
# Or configure the parts individually instead of POSTGRES_URL:
...
# Apply pending migrations on startup (disable in production)
...
| `data-color` | Primary theme color | `#2745F8` |
...
- **Database**: Use a dedicated role and require SSL (`POSTGRES_URL=postgresql://user:pass@host:5432/elo?sslmode=require`)
...
Mock external dependencies (vector store, LLM APIs) in all test types. Data-layer
tests in `backend/tests/providers/test_database.py` run against a real PostgreSQL
database instead — set `TEST_POSTGRES_URL` to point at a scratch database. They
...
Built with ❤️ using FastAPI, LangChain, and PostgreSQL
