...
const SIDEBAR_COLLAPSED_KEY = 'elo-sidebar-collapsed';
...
function getAppearanceTabContent() {
    ...
    const primaryColor = appearance.primary_color || '#2745F8';
                ...
                <div class="widget-preview-wrapper">
                    <div class="widget-preview" id="widget-preview">
                        ...
                        <div class="widget-preview-branding" id="preview-branding" style="${appearance.hide_branding ? 'display:none' : ''}">
                            <span id="preview-branding-text">Powered by elo</span>
                        </div>
                    </div>
...
function resetPreviewMessages() {
    ...
    const color = document.getElementById('config-color')?.value || '#2745F8';
...
async function checkHealth() {
    const checks = [
        { id: 'health-database', name: 'database' },
            ...
            datasets: [
                {
                    ...
                    data: conversations,
                    borderColor: '#2745F8',
                    backgroundColor: 'rgba(39, 69, 248, 0.08)',
                    borderWidth: 2,
                    pointRadius: currentPeriod === '7d' ? 4 : 2,
                    pointHoverRadius: 6,
                    pointBackgroundColor: '#2745F8',
                    tension: 0.35,
                    fill: true
                },
                {
                    ...
                    data: messages,
                    borderColor: '#6A78FF',
                    backgroundColor: 'rgba(106, 120, 255, 0.04)',
                    borderWidth: 2,
                    borderDash: [4, 4],
                    pointRadius: currentPeriod === '7d' ? 4 : 2,
                    pointHoverRadius: 6,
                    pointBackgroundColor: '#6A78FF',
                    tension: 0.35,
                    fill: false
                }
            ]
...
function updateWidgetPreview() {
    ...
    if (previewBranding) {
        ...
        if (whiteLabelEnabled) {
            ...
            // White-label disabled - show default elo branding
            previewBranding.classList.remove('hidden');
            if (link) {
                link.innerHTML = 'Powered by <strong>elo</strong>';
...
function populateConfigForm(config) {
    ...
    const colorInput = document.getElementById('config-color');
    const colorTextInput = document.getElementById('config-color-text');
    if (colorInput) colorInput.value = appearance.primary_color || '#2745F8';
    if (colorTextInput) colorTextInput.value = (appearance.primary_color || '#2745F8').toUpperCase();
...
function getConfigFromForm() {
    return {
        appearance: {
            primary_color: document.getElementById('config-color')?.value || '#2745F8',
...
function populateWhiteLabelForm(config) {
    const fields = {
        'wl-app-name': config.app_name || 'elo',
        'wl-primary-color': config.primary_color || '#2745F8',
        'wl-primary-color-text': config.primary_color || '#2745F8',
...
function applyWhiteLabelBranding(config) {
    ...
    if (config.app_name && config.app_name !== 'elo') {
...
async function saveWhiteLabelConfig() {
    ...
    try {
        const config = {
            app_name: document.getElementById('wl-app-name')?.value || 'elo',
            primary_color: document.getElementById('wl-primary-color')?.value || '#2745F8',
...
// ----- Avatar helpers -----
const AVATAR_COLORS = ['#2745F8','#3b82f6','#8b5cf6','#f97316','#10b981','#f59e0b','#ef4444','#4F63FF'];
