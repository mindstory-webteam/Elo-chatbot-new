/**
 * Widget Tests - elo Chatbot Widget
...
describe('elo Widget', () => {
  ...
  beforeEach(() => {
    ...
    mockScript.dataset.primaryColor = '#2745F8';
  ...
  describe('Widget Initialization', () => {
    test('should create widget container in DOM', () => {
      ...
      widget.className = 'elo-widget';
      document.body.appendChild(widget);

      expect(document.querySelector('.elo-widget')).toBeInTheDocument();
    });

    test('should create toggle button', () => {
      const widget = document.createElement('div');
      widget.className = 'elo-widget';
      widget.innerHTML = '<button class="elo-toggle" aria-label="Toggle chat"></button>';
      document.body.appendChild(widget);

      const toggle = document.querySelector('.elo-toggle');
    ...
    test('should create chat window', () => {
      const widget = document.createElement('div');
      widget.className = 'elo-widget';
      widget.innerHTML = '<div class="elo-window"></div>';
      document.body.appendChild(widget);

      expect(document.querySelector('.elo-window')).toBeInTheDocument();
    });
    ...
    test('should initialize with default config values', () => {
      const config = {
        ...
        primaryColor: mockScript.dataset.primaryColor || '#2745F8',
      ...
      expect(config.primaryColor).toBe('#2745F8');
  ...
  describe('Configuration Loading', () => {
    ...
    test('should use default values when config is unavailable', () => {
      ...
      expect(defaultConfig.primaryColor).toBe('#2745F8');
  ...
  describe('Branding Customization', () => {
    test('should show default branding when white-label disabled', () => {
      ...
      brandingEl.className = 'elo-branding';
      brandingEl.innerHTML = '<a href="https://example.com">Powered by <strong>elo</strong></a>';
      ...
      expect(brandingEl.querySelector('a').textContent).toContain('elo');
    });
  ...
  describe('Toggle Functionality', () => {
    test('should toggle open state when button clicked', () => {
      ...
      widget.className = 'elo-widget';
      widget.innerHTML = `
        <button class="elo-toggle"></button>
        <div class="elo-window"></div>
      `;
      document.body.appendChild(widget);

      const toggle = widget.querySelector('.elo-toggle');
      const chatWindow = widget.querySelector('.elo-window');
      let isOpen = false;
